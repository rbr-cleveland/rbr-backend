...
var whitelist = ['http://localhost:4000', 'https://rbr-cleveland.github.io', 'https://rustbeltriderscomposting.com', 'https://www.rustbeltriderscomposting.com'];
var corsOptions = {
...
// FUCK IT, heroku, and multiline environment variables
var accountInfo = {
    ...
    if (err) {
      res.status(500).json({
        ...
        rbrSheet.addRow(1, row, function(err, newRow) {
          if (err) {
            res.status(500).json({
              message: "There was an error adding the row",
              error: err
            })
          } else {
